...
def classify_many(
    ...
    **kwargs,
    ...
    журналов отмены. Об упавшей пачке надо сказать вслух, и это не мелочь.
    Молча она выглядит как обычный ответ: окно считает оставшихся без решения
    вычитанием (сколько спросили минус сколько правил вышло) и печатает «без
    решения: 40» — фразу, которая значит «модель посмотрела и не смогла».
    На деле модель этих сорока имён не видела вовсе: оборвалась сеть, истёк
    таймаут, ключ упёрся в предел запросов. Разница вся: в первом случае делать
    нечего, во втором помогает повторное нажатие — а понять, какой из двух
    случаев перед тобой, было неоткуда.
    ...
    for index, batch in enumerate(batches, start=1):
        ...
        try:
            ...
            if problems is not None:
                problems.append(
                    f"пачка {index} из {len(batches)} ({len(batch)} имён) не "
                    f"дошла до модели: {exc}")
        if on_progress:
