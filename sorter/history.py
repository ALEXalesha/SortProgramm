...
def undo_operation(op: Operation, config=None) -> list[tuple[str, str]]:
    ...
    try:
        ...
        # Файлы вернулись, а журнал остался лежать как был — и это меняет смысл
        # всей записи. Сортировка висит в «🕘 Истории» неоткатанной, а повторная
        # отмена берётся за файлы, которые уже дома: по новому пути к этому
        # времени может стоять тёзка, и второй откат унесёт в корень её.
        #
        # Молчали об этом до сих пор единственным местом во всей отмене. Лежит
        # журнал в папке пользователя, так что причины самые обычные: файл
        # только для чтения, его держит антивирус, кончилось место. Потеря или
        # порча возможности откатить — ошибка, о которой `apply` кричит
        # отдельной строкой; здесь она была тихой.
        notes.append((
            op.log_path.name,
            f"журнал отмены не переписан ({exc}). Файлы вернулись, но эта "
            "сортировка останется в истории как неоткатанная — отменять её "
            "второй раз не надо."))
    return notes
