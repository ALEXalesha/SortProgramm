...
def undo_operation(op: Operation, config=None) -> list[tuple[str, str]]:
    ...
    Что вернулось, а что нет, спрашиваем у файловой системы, и спрашивать надо
    про оба конца записи. Пустого места по новому пути мало: его освобождает и
    откат, и следующая сортировка, которая унесла файл дальше. А это самый
    обычный порядок работы — прибрались, поправили правила, нажали
    «Переразложить старое», — и в «🕘 Истории» такие сортировки стоят двумя
    строками подряд. Выбрав нижнюю (ту, с которой всё началось), человек
    получал оговорку «возвращать нечего» и стёртый журнал: файл никуда не
    вернулся, а запись, по которой его можно было бы вернуть после отката
    верхней строки, исчезла навсегда. Цепочка рвалась молча и необратимо —
    ровно та потеря возможности откатить, которую эта программа считает
    ошибкой.

    Поэтому запись уходит из журнала, только если по новому пути пусто И по
    исходному файл есть: вместе это и значит «вернулся».
    """
    notes = _undo(op.log_path, config)
    left = [e for e in op.entries
            if Path(e["dst"]).exists() or not Path(e["src"]).exists()]
    try:
        if left:
