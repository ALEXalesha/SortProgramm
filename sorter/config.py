...
# Начало жалобы на ненастроенную папку загрузок. Вынесено сюда, потому что
# читателей у неё двое: тот, кто её печатает, и консоль с `--path`, которая эту
# самую настройку только что перебила и потому обязана жалобу снять
# (`main.run_cli`). Сверяться с куском текста, набранным во второй раз, значит
# однажды не совпасть — ровно та беда, из-за которой общим сделан и
# `path_3d_reason`.
NO_DOWNLOADS_PROBLEM = "config.json: папка загрузок не указана."
...
# Символы, которых не бывает в ПУТИ. Набор уже, чем у имени папки, и нарочно:
# в пути законны и двоеточие (буква диска), и `?` (расширенный префикс
# `\\?\C:\...`, которым Windows снимает предел длины). Управляющие символы не
# законны нигде — файловая система их не примет ни в одном виде.
_CONTROL_IN_PATH = re.compile(r"[\x00-\x1f]")
...
def usable_3d_path(raw) -> bool:
    ...
    Управляющий символ в пути отбивается здесь же, и это не придирка, а починка
    падения. Проверка тут стояла на ФОРМЕ пути («абсолютный ли»), но ни разу —
    на том, примет ли такой путь файловая система. `Path("C:/All\\x003d")`
    считается абсолютным, `is_dir()` на нём отвечает False молча, а `mkdir`
    бросает `ValueError` — не `OSError`, поэтому его не ловил никто по дороге:
    `apply` ждёт `OSError`, и исключение уходило наружу. В консоли это стек
    вместо отчёта, в окне PyQt хуже — необработанное исключение в слоте гасит
    процесс целиком, без единого слова, и часть файлов к тому времени уже
    переехала.

    Приходит такой символ обычным путём: склеенная программой строка, съехавшая
    замена в редакторе, перенос настроек с другой машины. README обещает, что
    испорченные настройки программу не роняют, — обещание держится здесь.

    Набор символов уже, чем у имени папки (`_CONTROL_IN_PATH` против
    `_FORBIDDEN_IN_NAME`), и это нарочно: `?` в пути законен — с него
    начинается расширенный префикс `\\\\?\\C:\\...`, которым Windows снимает
    предел длины пути, и такой путь работает.
    """
    return (isinstance(raw, str) and bool(raw)
            and not _CONTROL_IN_PATH.search(raw) and Path(raw).is_absolute())
...
def path_3d_reason(raw) -> str:
    ...
    bad = _CONTROL_IN_PATH.search(raw) if isinstance(raw, str) else None
    if bad:
        # Сам символ не печатаем: он невидимый, и строка отчёта от него
        # разъезжается. Называем его так же, как это делает проверка имени
        # папки, — `\x00` вместо ничего.
        shown = repr(bad.group())[1:-1]
        return (f"в пути есть символ «{shown}», которого файловая система в "
                "имени не разрешает — по такому пути не создать ни папки")
...
def _text_map(
    ...
    """Раздел вида «имя → строка». Сейчас такой один — `overrides`.
    ...
    является. `explain_category` берёт её через `or`, то есть молча
    проваливается дальше к шаблонам и словам, и отличить такую запись от
    «правила и не было» нельзя ничем — при том, что писали её руками.
    """
    if raw is None:
        return {}
    if not isinstance(raw, dict):
...
def _drop_frozen_rules(
    ...
    notices.append(
        ...
        "не пускает никогда. Убери их из файла, чтобы сообщение больше "
        "не появлялось.")
    return {name: category for name, category in overrides.items()
            if name not in set(frozen)}


def _check_overridden_patterns(
    overrides: dict[str, str],
    patterns: dict[str, list[str]],
    where: str,
    notices: list[str],
) -> None:
    """Говорит про ручное правило, которое спорит с шаблоном.
    ...
    (`explain_category`), поэтому молча уводит такой файл куда угодно, и заметить
    это можно только сравнив «куда легло» с «что сказал бы шаблон».

    Нашлось это на настоящей папке: пять файлов, четыре из них с датой в имени и
    один прямо со словом «Снимок экрана», лежали в «Дизайне» — а в «Скриншотах»
    оставалось два файла. Правила эти писал не человек: их насыпала кнопка
    «✨ИИ», пока была. Она отвечала за файл, про который её спросили, и ни разу
    не спрашивала, есть ли у программы собственное мнение.

    Спорить с шаблоном бывает и правильно — скриншот может лежать в «Дизайне»
    нарочно, как референс. Поэтому это `notices`, а не `problems`: раскладка не
    сломана, но человек должен знать, что его правило перебивает уверенное
    решение. Сказать надо один раз при старте, а не при каждой уборке.

    Спрашиваем только про ШАБЛОНЫ, не про слова. Слово — приём грубый: `python`
    в `Код` ловит `python-3.13.7-amd64.exe`, а человек хочет его в «Программах»,
    и таких правил дюжина. Жаловаться на них значило бы шуметь ровно там, где
    ручное правило и делает свою работу — уточняет грубое совпадение.
    """
    # `classifier` импортирует `config`, поэтому здесь только отложенный импорт:
    # свой второй `match_pattern` разошёлся бы с настоящим на первой же правке.
    from .classifier import base_name, match_pattern

    clashes = []
    for name, category in overrides.items():
        by_pattern = match_pattern(base_name(name), patterns)
        if by_pattern and folder_key(by_pattern) != folder_key(category):
            clashes.append((name, by_pattern, category))
    if not clashes:
        return
    shown = ", ".join(f"«{n}» ({p} → {c})" for n, p, c in clashes[:3])
    tail = f" и ещё {len(clashes) - 3}" if len(clashes) > 3 else ""
    notices.append(
        f"{where}: ручное правило спорит с шаблоном ({len(clashes)} шт.): "
        f"{shown}{tail}. Шаблон узнаёт такой файл по имени наверняка, а правило "
        "стоит выше и уводит его в другую категорию. Если так и задумано — всё "
        "в порядке; если нет, убери правило, и файл поедет по шаблону.")
...
class Config:
    ...
    def load(cls, path: str | Path, *, user: bool = True) -> "Config":
        ...
        # Незнакомые ключи возвращаются в файл при сохранении, и правила,
        # оставшиеся в `config.json` от старой версии, — тоже незнакомые:
        # читаем мы их из `rules.json`, а писать на их место нечего.
        #
        # Раньше здесь стояло `RULE_KEYS + USER_KEYS`, то есть разделы правил
        # выбрасывались из `extra` — и первое же закрытие окна стирало их из
        # файла насовсем. Дорога сюда одна и обычная: обновление. Установщик
        # кладёт рядом `rules.json` (`ignoreversion`), а `config.json` не
        # трогает (`onlyifdoesntexist`), так что у обновившегося пользователя
        # оказываются оба файла — и с этой минуты его собственные категории,
        # шаблоны и `type_map`, которые README прошлой версии предлагал править
        # именно в `config.json`, живут ровно до следующего закрытия окна.
        # Стирались они молча и без второй копии, при том что соседняя строка
        # «моя_заметка», дописанная рукой туда же, переживала сохранение.
        skip = USER_KEYS

        # Правила в `config.json` при живом `rules.json` не действуют, и об
        # этом надо сказать. Молчание тут — та же беда, что и у всего
        # остального в этом файле: снаружи всё исправно. Человек правит
        # категории там, где их правил всегда, перезапускает программу и видит
        # прежнюю раскладку — ни ошибки, ни намёка на то, что читают теперь
        # другой файл. Строки мы больше не стираем (см. `skip`), так что
        # перенести их в `rules.json` можно хоть завтра — но узнать, что
        # переносить надо, неоткуда, кроме этой жалобы.
        if not inline_rules:
            stale = [key for key in RULE_KEYS if key in data]
            if stale:
                problems.append(
                    f"{path.name}: правила внутри него больше не действуют — "
                    f"они читаются из {RULES_FILENAME} ({', '.join(stale)}). "
                    "Строки оставлены как есть: свои правки перенеси в "
                    f"{RULES_FILENAME}, иначе они не применятся.")
        ...
        # Раскладывать по типам не по чему. Раскладка у программы двухэтажная —
        # `Категория/Тип/файл`, — и без карты типов второй этаж исчезает: тип
        # всем выдаётся запасной, то есть все загрузки ложатся в
        # `Категория/Misc`. Снаружи это выглядит исправной работой: план
        # построен, категории выбраны верно, жалоб нет, и отличить такую
        # раскладку от честной («у всех файлов и правда неизвестные
        # расширения») нельзя ничем.
        #
        # Дорога сюда та же, что у пустого `managed_folders`: раздел стёрли при
        # правке руками, не довезли при переносе настроек, потеряли при слиянии
        # файлов. Испорченный раздел разбор называет своими словами
        # (`_rule_map`), а про отсутствующий и пустой не говорил ничего — тот
        # самый третий вход валидатора, про который забывают: правильный,
        # испорченный и отсутствующий.
        if not type_map:
            problems.append(
                f"{rules_name}: type_map пуст — тип файла определять нечем. "
                f"Все файлы лягут в «Категория/{fallback_type}», без разбора "
                "по типам.")
    ...
    def save(self, path: str | Path) -> None:
        ...
        своё в config.json, сохранение из окна не должно это стирать. Разделы
        правил, оставшиеся в нём от старой версии, считаются такими же
        незнакомыми — читаются они из `rules.json`, а стирать чужую правку
        сохранение не вправе (см. `skip` в `load`). Своих правил окно в
        `config.json` не пишет никогда: в `extra` попадает только то, что уже
        лежало в файле.
