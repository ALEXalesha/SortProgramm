...
#
# `type_map` тут по той же причине. Раскладка у программы двухэтажная, и без
# карты типов второй этаж исчезает: тип всем выдаётся запасной, все загрузки
# ложатся в `Категория/Misc`. Разбор теперь про это говорит — а «здоровый»
# конфиг, на котором проверяют молчание разбора, до сих пор был как раз таким.
RULES = {
